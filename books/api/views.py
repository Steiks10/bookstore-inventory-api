...
class BookView(APIView):
    ...
    def put(self, request, id: int):
        try:
          ...
          # Allow partial updates so unchanged fields aren't required
    ...
    def get(self, request):
        try:
            ...
            if threshold is None:
                return Response({'detail': 'threshold es requerido'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            page_size = int(page_size) if page_size is not None else None
            repo = book_repository_provider.get()
            dto = list_low_stock_books(repository=repo, threshold=threshold, page=page, page_size=page_size)
            data = {
                'items': [
                    {
                        'id': book.id,
